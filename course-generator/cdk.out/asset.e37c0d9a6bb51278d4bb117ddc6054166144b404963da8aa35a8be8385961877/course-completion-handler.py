...
def handle_content_generation(event):
    """Handle content generation using Amazon Titan Text"""
    try:
        ...
        # Prepare full prompt for Titan
        full_prompt = f"""You are an expert {role} for language education. Create detailed content for a {language} course at {level} CEFR level.
...
Please provide comprehensive, structured content that follows educational best practices and CEFR standards. Include specific examples, activities, and assessments suitable for {level} level learners."""

        # Call Amazon Titan via Bedrock
        ...
        generated_content = response_body['results'][0]['outputText']
        ...
        # Fallback to basic content if Bedrock fails
        generated_content = f"Error generating content for {role} in {language} {level} course. Bedrock access required."
